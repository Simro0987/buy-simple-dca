  ...
  const opportunities = useMemo<OpportunityResult[]>(() => {
    ...
    const ops: OpportunityInput[] = [
      ...
      },
      {
        ...
        asset: 'ETH',
        color: '#627EEA',
        title: lang === 'sk' ? 'wstETH cez DeFi Saver (ARB)' : 'wstETH via DeFi Saver (ARB)',
        desc: lang === 'sk' ? 'ETH → wstETH (ARB) → DeFi Saver (bez páky)' : 'ETH → wstETH (ARB) → DeFi Saver (no leverage)',
        apy: 9,
      ...
      {
        id: 'jitosol-sol-kamino-lp',
        asset: 'SOL',
        color: '#9945FF',
        title: lang === 'sk' ? 'jitoSOL/SOL LP v Kamino (bez páky)' : 'jitoSOL/SOL LP in Kamino (no leverage)',
        desc: lang === 'sk' ? 'jitoSOL + SOL → Kamino Liquidity pool' : 'jitoSOL + SOL → Kamino Liquidity pool',
        apy: 8,
        feeUsd: gas.fees.solSwap + gas.fees.solDeposit,
      },
    ];
